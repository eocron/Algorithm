...
namespace Eocron.Algorithms.Intervals
{
    ...
    {
        [DataMember(Order = 1, Name = "s")] public readonly IntervalPoint<T> StartPoint;

        [DataMember(Order = 2, Name = "e")] public readonly IntervalPoint<T> EndPoint;

        [IgnoreDataMember] public bool HasStart => !StartPoint.IsInfinity;
        [IgnoreDataMember] public bool HasEnd => !EndPoint.IsInfinity;
            ...
            IComparer<IntervalPoint<T>> comparer = null)
        {
            ...
            if (cmp > 0)
                throw new ArgumentOutOfRangeException(nameof(startPoint),
                    "startPoint should be lower or equal to endPoint");
