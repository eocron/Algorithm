...
namespace Eocron.Algorithms.FileCache.Async
{
    ...
    {
        Task AddOrUpdateFileAsync(TKey key, string sourceFilePath, CancellationToken token,
        ...
        /// <param name="policy">
        ///     Caching policy to apply to cached item. Null if no policy needed, and item invalidation performed
        ///     by user.
        /// </param>
        ...
        Task GetFileOrAddFileAsync(TKey key, Func<TKey, Task<string>> provider, CancellationToken token,
            string targetFilePath, ICacheExpirationPolicy policy);

        Task GetFileOrAddStreamAsync(TKey key, Func<TKey, Task<Stream>> provider, CancellationToken token,
            string targetFilePath, ICacheExpirationPolicy policy);
        ...
        /// </param>
        /// <returns></returns>
        Task<Stream> GetStreamOrAddFileAsync(TKey key, Func<TKey, Task<string>> provider, CancellationToken token,
            ICacheExpirationPolicy policy = null);
        ...
        /// <param name="token"></param>
        /// <param name="policy">
        ///     Caching policy to apply to cached item. If item expired it will be removed from cache at GC. Null
        ///     if no policy needed and item invalidation performed by user.
        /// </param>
        /// <returns></returns>
        Task<Stream> GetStreamOrAddStreamAsync(TKey key, Func<TKey, Task<Stream>> provider, CancellationToken token,
            ICacheExpirationPolicy policy = null);
